...
class SpotlightIndexClientTests(unittest.TestCase):
    def capture_request(self, body: dict | bytes = None):
        ...
        def fake_urlopen(request, timeout):
            ...
            captured["origin"] = request.get_header("X-origin")
    ...
    def test_search_response_sends_payload_and_maps_results(self) -> None:
        ...
        client = SpotlightIndexClient(originator_app="Codex")
        ...
        self.assertEqual(captured["origin"], "Codex")
        self.assertEqual(
            captured["body"],
            {
                "query": "passport",
    ...
    def test_thumbnail_by_id_uses_id_query_parameter_and_origin(self) -> None:
        captured, fake_urlopen = self.capture_request(b"\x00\xffthumbnail")

        client = SpotlightIndexClient(base_url="http://localhost:9999///", originator_app="Claude Code")
        with patch("spotlight_index_client.urlopen", fake_urlopen):
        ...
        self.assertIsNone(captured["authorization"])
        self.assertEqual(captured["origin"], "Claude Code")
    ...
    def test_simple_endpoint_contracts(self) -> None:
        cases = [
            (
                ...
                "/health",
                None,
                {"status": "ok", "spotlightIndexingEnabled": True, "providers": ["files"]},
            ),
            (
                "providers",
                lambda client: client.providers(),
                "GET",
                "/v1/providers",
                None,
                {
                    "providers": [
                        {
                            ...
                            "setupHint": None,
                            "checks": [{"name": "spotlight-indexing", "status": "ok", "message": "ready"}],
                        }
                    ]
                },
            ),
            (
                ...
                None,
                {
                    ...
                    "metadataAttributes": ["kMDItemDisplayName"],
                    "providerFields": {
                        "files": {
                            "entityTypes": ["file"],
                            "fields": {"title": "Title"},
                            "metadataFields": {"kMDItemDisplayName": "Display name"},
                        }
                    },
                },
            ),
        ...
        for name, call, method, path, body, response in cases:
            with self.subTest(name=name):
                ...
                client = SpotlightIndexClient(base_url="http://localhost:9999///", originator_app="OpenCode")
                with patch("spotlight_index_client.urlopen", fake_urlopen):
                    result = call(client)
                ...
                if name == "health":
                    ...
                    self.assertTrue(result.spotlight_indexing_enabled)
                    self.assertEqual(result.providers, ["files"])
                elif name == "providers":
                    self.assertIsInstance(result, ProvidersResponse)
                    self.assertEqual(result.providers[0].source, "files")
                    self.assertEqual(result.providers[0].checks[0].name, "spotlight-indexing")
                elif name == "schema":
                    self.assertIsInstance(result, SchemaResponse)
                    self.assertEqual(result.supported_sources, ["files"])
                    self.assertEqual(result.provider_fields["files"].entity_types, ["file"])
                else:
                ...
                self.assertIsNone(captured["authorization"])
                self.assertEqual(captured["origin"], "OpenCode")
