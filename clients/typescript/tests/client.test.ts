...
test("searchResponse sends payload and maps response", async () => {
  ...
  const client = new LimelightClient({ fetch, originatorApp: "Codex" });
  ...
  assert.equal((calls[0].init.headers as Headers).get("X-Origin"), "Codex");
  assert.equal((calls[0].init.headers as Headers).get("Authorization"), null);
...
test("photoThumbnail uses id query parameter and includes origin", async () => {
  const bytes = new Uint8Array([0, 255, 42]);
  const { fetch, calls } = makeFetch(new Response(bytes));

  const client = new LimelightClient({ baseUrl: "http://localhost:9999///", fetch, originatorApp: "Claude Code" });
  ...
  assert.equal((calls[0].init.headers as Headers).get("X-Origin"), "Claude Code");
  assert.equal((calls[0].init.headers as Headers).get("Authorization"), null);
...
test("simple endpoint contracts use expected routes and bodies", async () => {
  ...
  for (const scenario of cases) {
    ...
    const client = new LimelightClient({ baseUrl: "http://localhost:9999///", fetch, originatorApp: "OpenCode" });
    ...
    assert.equal((calls[0].init.headers as Headers).get("X-Origin"), "OpenCode", scenario.name);
    assert.equal((calls[0].init.headers as Headers).get("Authorization"), null, scenario.name);
