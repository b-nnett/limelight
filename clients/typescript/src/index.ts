...
export const DEFAULT_LIMELIGHT_BASE_URL = "http://127.0.0.1:8765";

export type KnownSearchSource =
  | "files"
  | "photos"
  | "contacts"
  | "calendar"
...
export type SearchSource = KnownSearchSource | (string & {});

export type KnownSearchType =
  | "application"
  | "document"
  | "image"
  | "audio"
...
export type SearchType = KnownSearchType | (string & {});

export type ProviderStatus = "ok" | "unavailable" | (string & {});
export type ProviderReadinessStatus = "ready" | "partial" | "needs_permission" | "missing" | "unavailable" | "unknown" | (string & {});
export type PermissionActionStatus = "opened" | "needs_manual_setup" | "unsupported" | (string & {});
...
export interface ProviderReadinessCheck {
  name: string;
  status: ProviderReadinessStatus | "ok" | "failed";
  path?: string | null;
  message?: string | null;
}
...
export interface SourceCapability {
  ...
  liveStatus: ProviderReadinessStatus;
  summary: string;
...
export class LimelightClient {
  ...
  private readonly originatorApp?: string;
  private readonly fetchImpl: typeof fetch;

  constructor(options: LimelightClientOptions = {}) {
    this.baseUrl = (options.baseUrl ?? DEFAULT_LIMELIGHT_BASE_URL).replace(/\/+$/, "");
    this.originatorApp = options.originatorApp;
  ...
  private async request<T>(method: string, path: string, payload?: unknown): Promise<T> {
    ...
    if (this.originatorApp) {
      headers.set("X-Origin", this.originatorApp);
    }
