...
public enum SpotlightSearchError: Error, LocalizedError {
    ...
    case ocrFileTooLarge(path: String, bytes: Int64, limit: Int64)
    case ocrImageTooLarge(path: String, width: Int, height: Int, maxPixels: Int, maxDimension: Int)
    ...
    public var errorDescription: String? {
        ...
        case .ocrFileTooLarge(let path, let bytes, let limit):
            "OCR image file is too large: \(path) is \(bytes) bytes, limit is \(limit) bytes"
        case .ocrImageTooLarge(let path, let width, let height, let maxPixels, let maxDimension):
            "OCR image dimensions are too large: \(path) is \(width)x\(height), limit is \(maxPixels) pixels and \(maxDimension) px per side"
