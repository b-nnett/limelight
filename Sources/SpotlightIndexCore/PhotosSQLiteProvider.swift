...
struct PhotosSQLiteProvider: SearchProvider {
    ...
    func search(_ context: ProviderSearchContext) throws -> [SearchResultRecord] {
        ...
        let assetColumns = try columnNames(table: "ZASSET", in: photosDB)
        var results: [SearchResultRecord] = []
        var seen: Set<String> = []

        for record in try personResults(context, photosDB: photosDB, assetColumns: assetColumns) {
        ...
        if FileManager.default.fileExists(atPath: searchDBPath) {
            ...
            for record in try lexemeResults(context, photosDB: photosDB, searchDB: searchDB, assetColumns: assetColumns) {
                if seen.insert(record.id).inserted {
                    results.append(record)
                }
                if results.count == context.limit { return results }
    ...
    private func personResults(_ context: ProviderSearchContext, photosDB: SQLiteDatabase, assetColumns: Set<String>) throws -> [SearchResultRecord] {
        ...
        for person in people {
            ...
            let assets = try photosDB.rows(
                """
                SELECT \(assetSelectList(alias: "a", columns: assetColumns))
    ...
    private func lexemeResults(_ context: ProviderSearchContext, photosDB: SQLiteDatabase, searchDB: SQLiteDatabase, assetColumns: Set<String>) throws -> [SearchResultRecord] {
        let query = context.query.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !query.isEmpty else { return [] }
        ...
        for (uuid, reason) in assetUUIDs {
            let rows = try photosDB.rows(
                """
                SELECT \(assetSelectList(alias: nil, columns: assetColumns))
                FROM ZASSET
                WHERE ZUUID = ?
    ...
    private func assetRecord(_ row: [String: SQLiteValue], matchReason: String, subtitle: String) -> SearchResultRecord? {
        ...
        let path = assetResolver.bestAssetPath(uuid: uuid, filename: filename, directory: row["ZDIRECTORY"]?.string)
    ...
    private func assetSelectList(alias: String?, columns: Set<String>) -> String {
        let prefix = alias.map { "\($0)." } ?? ""
        var selections = [
            "\(prefix)ZUUID AS ZUUID",
            "\(prefix)ZFILENAME AS ZFILENAME",
            "\(prefix)ZUNIFORMTYPEIDENTIFIER AS ZUNIFORMTYPEIDENTIFIER",
            "\(prefix)ZWIDTH AS ZWIDTH",
            "\(prefix)ZHEIGHT AS ZHEIGHT",
            "\(prefix)ZDATECREATED AS ZDATECREATED"
        ]
        if columns.contains("ZDIRECTORY") {
            selections.append("\(prefix)ZDIRECTORY AS ZDIRECTORY")
        } else {
            selections.append("NULL AS ZDIRECTORY")
        }
