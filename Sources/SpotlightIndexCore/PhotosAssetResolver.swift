...
struct PhotosAssetResolver {
    ...
    func bestAssetPath(uuid: String, filename: String) -> String? {
        resolveDerivativePath(uuid: uuid) ?? resolveOriginalPath(filename: filename)
    }
    ...
    private func resolveOriginalPath(filename: String) -> String? {
    ...
    private func resolveDerivativePath(uuid: String) -> String? {
        ...
        for folder in folders {
            ...
            if let match = files.first(where: { $0.lastPathComponent.hasPrefix(uuid) && ["jpeg", "jpg", "png", "heic"].contains($0.pathExtension.lowercased()) }),
