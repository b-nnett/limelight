...
`originator_app` is sent as `X-Origin` so Limelight can show where searches came
from in its recent-search UI. Limelight does not use bearer tokens.
...
response = client.extract(
    ...
    save_to="~/Documents/document-number.txt",
)
```
