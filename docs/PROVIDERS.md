...
- `notes`: local Notes SQLite/private-store fallback when accessible
...
Mail, Notes, and Safari also try CoreSpotlight app-entity queries before direct private-store reads. Mail app-entity search is additionally gated by Apple's private `com.apple.corespotlight.search.allow.mail` entitlement, which cannot be made usable with a simple ad-hoc signature.
