...
```sh
curl -s -H 'X-Origin: Example Agent' http://127.0.0.1:8765/v1/providers | jq
```

Provider readiness statuses include `ready`, `partial`, `needs_permission`, and `missing`, with setup hints for protected stores.

The generated [Capability Matrix](CAPABILITY_MATRIX.md) is a local runtime snapshot from `/v1/capabilities`; readiness reflects the current machine, user account, permissions, and data stores.
