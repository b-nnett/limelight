...
cat > "$TMP_DIR/Limelight.entitlements" <<'PLIST'
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
...
codesign --force --deep --entitlements "$TMP_DIR/Limelight.entitlements" --sign "$CODESIGN_IDENTITY" "$APP_DIR"
...
echo "HTTP access: loopback-only; local clients identify themselves with X-Origin."
